...
to setup
  ...
  ask patches with [pxcor = 0 or pxcor = 20 or pycor = 0 or pycor = 20] [ set pcolor black ]

  if(obstacles)
  [
    ask patch 8 17 [ set pcolor black ]
    ask patch 13 17 [ set pcolor black ]
  ]

  (ifelse
    exit-width = 1
    [
      ask patches with [pxcor > 9 and pxcor < 11 and pycor = 20] [ set pcolor blue ]
    ]
    exit-width = 2
    [
      ask patches with [pxcor > 9 and pxcor < 12 and pycor = 20] [ set pcolor blue ]
    ]
    exit-width = 3
    [
      ask patches with [pxcor > 8 and pxcor < 12 and pycor = 20] [ set pcolor blue ]
    ]
    exit-width = 4
    [
      ask patches with [pxcor > 7 and pxcor < 12 and pycor = 20] [ set pcolor blue ]
    ]
    exit-width = 5
    [
      ask patches with [pxcor > 7 and pxcor < 13 and pycor = 20] [ set pcolor blue ]
    ]
  )

  ask patch 10 20 [ ask patches with [pcolor = white] in-radius 6 [ set pcolor 7.8 ] ]
  ...
  create-evacuees population [
    move-to one-of patches with [pcolor = white and pycor < 17]
    set size 1
    set color red

    set state 2
    set walking-speed 1
    set reaction-timer random 30

    set list_preevac fput reaction-timer list_preevac
    set destination one-of list_exit_queue
    calculate-path
  ]
...
to walk
  ...
  ifelse(distance destination < walking-speed or [pcolor = 7.8 or pcolor = 8.8] of patch-here)
  [
    move-to destination
    set state 1
...
to queue
  ...
  [
    set exit1_queue fput self exit1_queue
    set state "queue"
  ]
  [
...
to social-force
  ...
  (ifelse
    ...
    people-present > 7
    [
      ifelse(obstacles) [ set walking-speed 0.2 ] [set walking-speed 0.1]
    ]
    ;else
    [
...
to calculate-path
 ...
 [
  ...
  set calculated-path A* Start destination patches with [pcolor = white or pcolor = blue or pcolor = 7.8]
...
to calculate-path-queue
 ...
 [
  ...
  let calculated-path 0
  set calculated-path A* Start destination patches with [pcolor = blue or pcolor = 7.8]
...
to clear-queues
    set exit1_queue process-queue-exits exit1_queue exit-width
...
@#$#@#$#@
<experiments>
  <experiment name="Pre evac time" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean list_preevac</metric>
    <steppedValueSet variable="population" first="50" step="50" last="300"/>
  </experiment>
  <experiment name="flow throughput" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>ticks</metric>
  ...
  <experiment name="obstacles" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>ticks</metric>
    <enumeratedValueSet variable="exit-width">
