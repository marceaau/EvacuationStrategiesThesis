...
to setup
  ...
  ask patches with [pxcor = 0 or pxcor = 30 or pycor = 0 or pycor = 10] [ set pcolor black ]

  if(obstacles)
  [
    ask patch 13 7 [ set pcolor black ]
    ask patch 18 7 [ set pcolor black ]
  ]

  ask patches with [pxcor > 13 and pxcor < 17 and pycor = 10] [ set pcolor blue ]
  ask patch 15 10 [ ask patches with [pcolor = white] in-radius 6 [ set pcolor 7.8 ] ]
  ...
  create-evacuees population [
    move-to one-of patches with [pcolor = white and pycor < 12]
    set size 1
    set color red
